            ...
            <div className="relative z-10 max-w-4xl mx-auto px-6">
                ...
                <div className="text-center mb-16">
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase mb-6 border transition-colors duration-500
                            ${isDark
                                ? 'bg-amber-500/10 border-amber-500/20 text-amber-400'
                                : 'bg-amber-50 bg-white border-amber-200 text-amber-600'
                            }`}
                    >
                        <Crown size={14} className={isDark ? "fill-amber-400" : "fill-amber-600"} />
                        Excellence Arcade
                    </motion.div>
                    ...
                    <motion.h2
                        ...
                        className="text-4xl md:text-5xl font-black tracking-tight text-slate-900 dark:text-white mb-6 relative z-20"
                    >
                    ...
                    <motion.p
                        ...
                        className="max-w-xl mx-auto text-base text-slate-600 dark:text-slate-400 font-medium"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 items-start">
                    ...
                    {topProject && (
                        <FameCard
                            ...
                            accentColor="amber"
                            delay={0}
                            className="z-30 order-first md:order-none scale-110"
                            floatDuration={5}
                            id={topProject.id}
                            isDark={isDark}
                        />
...
const getAvatarUrl = (name: string) => {
    ...
    if (lower.includes('parth') || lower.includes('aditya') || lower.includes('krishna') || lower.includes('pranav') || lower.includes('rohit')) isFemale = false;
    if (lower.includes('sharwari') || lower.includes('uttara') || lower.includes('divya') || lower.includes('drishti')) isFemale = true;
...
const FameCard = ({ rank, title, name, project, description, icon, gradient, accentColor, delay, floatDuration = 4, className = "", id, isDark }: any) => {
        ...
        >
            ...
            >
                <motion.div
                    ...
                    className={`relative h-full w-full rounded-[2rem] border overflow-hidden flex flex-col transition-colors duration-500
                        ${isDark
                            ? 'bg-slate-900/60 backdrop-blur-3xl border-white/5 shadow-[0_0_30px_rgba(0,0,0,0.5)]'
                ...
                >
                    ...
                    <div className={`absolute top-0 inset-x-0 h-24 bg-gradient-to-br ${gradient} p-5 flex items-start justify-between z-0`}>
                        <div className={`relative z-10 backdrop-blur-md border p-2.5 rounded-xl shadow-lg transition-colors
                            ${isDark ? 'bg-black/20 border-white/10' : 'bg-white/20 border-white/30'}`}>
                            {icon}
                        </div>
                        <div className={`font-black text-6xl opacity-20 -mt-3 -mr-2 select-none transition-colors 
                    ...
                    <div className="relative pt-24 px-6 pb-6 flex flex-col items-center justify-between text-center z-10 h-full">
                        <div className={`absolute top-12 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-[3px] shadow-lg overflow-hidden z-20 group-hover:scale-110 transition-all duration-300
                            ${isDark ? 'border-slate-800 bg-slate-900' : 'border-white bg-white'}`}>
                            <img
                                src={avatarUrl}
                                alt={name}
                                className="w-full h-full object-cover bg-slate-50 dark:bg-slate-800"
                                loading="lazy"
                            />
                        </div>

                        <div className="mt-12 mb-2">
                            <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-1">{title}</h4>
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white leading-tight px-2">{name}</h3>
                        </div>

                        <div className={`w-10 h-1 rounded-full my-3 transition-colors ${isDark ? 'bg-slate-800 group-hover:bg-slate-700' : 'bg-slate-100 group-hover:bg-slate-200'}`} />

                        <div className="flex-1 flex flex-col justify-center w-full">
                            <p className={`font-bold text-sm mb-2 ${accentClasses[accentColor]}`}>{project}</p>
                            <p className="text-slate-500 dark:text-slate-400 text-xs leading-relaxed line-clamp-4 px-2">{description}</p>
                        </div>

                        <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className={`mt-4 px-6 py-2 rounded-xl font-bold text-xs transition-all flex items-center gap-2 shadow-sm
                                ${isDark
                                    ? 'bg-slate-800 text-white border-white/5 hover:bg-slate-700'
                                    : 'bg-slate-50 text-slate-900 border-slate-200 hover:bg-slate-100'
                                }`}
                        >
                            View Profile <Star size={12} className={isDark ? "fill-white" : "fill-slate-900"} />
                        </motion.button>
                    </div>

                    {/* Shine Effect */}
                    <div className="absolute inset-0 z-50 pointer-events-none bg-gradient-to-tr from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 w-[200%] h-[200%] -translate-x-full -translate-y-full group-hover:translate-x-full group-hover:translate-y-full" />
                </motion.div>
            </motion.div>
